...
class DRAMModelToCtrlFSMIO() extends Bundle {
  val readAddr = UInt(OUTPUT)
  val writeAddr = UInt(OUTPUT)
  val doRead = Bool(OUTPUT)
  val doWrite = Bool(OUTPUT)
  val writeData = Vec.fill(DEVICE_WIDTH){Bits(width = MEM_DATA_BITS)}.asOutput
  val readData = new Bundle {
    val valid = Bool().asInput
    val data = Vec.fill(DEVICE_WIDTH){Bits(width = MEM_DATA_BITS)}.asInput
  }
...
class DRAMTimingModel() extends Module {
  val io = new Bundle {
  ...
  if(ADDR_OFFSET_WIDTH > 0){
    io.ctrlFSM.readAddr := Cat(io.memController.cmdBus.bankAddr, rowAddr, io.memController.cmdBus.colAddr, UInt(0, width = ADDR_OFFSET_WIDTH))
  } else {
  ...
  if(BURST_LENGTH - 1 > 0){
    ...
    for(i <- 0 until DEVICE_WIDTH){
      io.ctrlFSM.writeData(i) := writeData((i+1)*MEM_DATA_BITS - 1, i*MEM_DATA_BITS)
    }
    when(io.ctrlFSM.fireTgtCycle){
    ...
    for(i <- 0 until DEVICE_WIDTH){
      io.ctrlFSM.writeData(i) := io.memController.writeDataBus.data((i+1)*MEM_DATA_BITS - 1, i*MEM_DATA_BITS)
    }
  }
  ...
  if(tWL + BURST_LENGTH - 1 > 0){
    ...
    when(io.ctrlFSM.fireTgtCycle){
      ...
      if(ADDR_OFFSET_WIDTH > 0){
        writeAddrShiftRegs(0) := Cat(io.memController.cmdBus.bankAddr, rowAddr, io.memController.cmdBus.colAddr, UInt(0, width = ADDR_OFFSET_WIDTH))
      } else {
    ...
    if(ADDR_OFFSET_WIDTH > 0){
      io.ctrlFSM.writeAddr := Cat(io.memController.cmdBus.bankAddr, rowAddr, io.memController.cmdBus.colAddr, UInt(0, width = ADDR_OFFSET_WIDTH))
    } else {
