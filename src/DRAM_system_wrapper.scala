...
class DRAMSystemWrapper() extends Module {
  val io = new Bundle{
    val mem = new ioMem()
    val memResp = new ioQueueFame1(new MemResp())
    val memReqCmd = new ioQueueFame1(new MemReqCmd()).flip()
    val memReqData = new ioQueueFame1(new MemData()).flip()
    val errors = new DRAMModelErrorIO()
    val params = new DRAMModelParameterIO()
    //debug
    //val ctrlFSMReadData = Vec.fill(DEVICE_WIDTH){Bits(width = MEM_DATA_BITS)}.asOutput
    //val memControllerReadDataIn = Vec.fill(DEVICE_WIDTH){new MemData}.asOutput
